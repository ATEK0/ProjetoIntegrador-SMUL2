...
class InterestRateTierSerializer(serializers.Serializer):
    from_period = serializers.IntegerField(
        min_value=1, help_text="Período inicial (1 = primeiro ano/mês)"
    )
    rate = serializers.FloatField(
        min_value=0.0, help_text="Taxa decimal a partir deste período (ex: 0.05)"
    )
...
class InterestSimulationSerializer(serializers.Serializer):
    ...
    principal = serializers.FloatField(
        min_value=0.01, help_text="Capital inicial (Principal)"
    )
    rate = serializers.FloatField(
        min_value=0.0,
        required=False,
    ...
    time = serializers.FloatField(
        min_value=0.1, help_text="Tempo da aplicação/empréstimo"
    )
    type = serializers.ChoiceField(
        choices=INTEREST_CHOICES, help_text="Tipo de juros (simple ou compound)"
    )
    rate_tiers = InterestRateTierSerializer(
...
class AmortizationSimulationSerializer(serializers.Serializer):
    ...
    type = serializers.ChoiceField(
        ...
        help_text="Tipo de amortização (french, sac ou american)",
    )
