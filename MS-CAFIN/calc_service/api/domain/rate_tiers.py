def normalize_rate_tiers(tiers: list[dict] | None) -> list[dict]:
    ...
    return sorted(
        [
            {"from_period": int(t["from_period"]), "rate": float(t["rate"])}
            for t in tiers
        ],
...
def validate_rate_tiers(tiers: list[dict], time: float) -> None:
    ...
    for t in tiers:
        if t["from_period"] > max_period:
            period = t["from_period"]
            raise ValueError(
                f"O período inicial {period} não pode ser superior ao "
                f"número total de períodos ({max_period})."
            )
